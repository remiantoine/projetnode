...
module.exports = class FilmService extends Service {
    ...
    async exportFilmsToCSV(userEmail) {
        const { Film } = this.server.models();
        ...
        try {
            ...
            // Convertir les films en CSV
            const parser = new Parser();
            const csv = parser.parse(films);
            const filePath = `films_${Date.now()}.csv`;
            ...
            const mailOptions = {
                ...
                text: 'Veuillez trouver en pièce jointe le fichier CSV contenant les films.',
                attachments: [
                    {
                        filename: 'films.csv',
                        path: filePath
