    ...
    },
    {
        ...
        handler: async (request, h) => {
            ...
            try {
                ...
                const mailService = request.server.services().mailService;
                const result = await mailService.sendMail(mailOptions);
                return h.response({ success: true, message: 'Email sent successfully', messageId: result.messageId }).code(200);
            } catch (err) {
                return h.response({ success: false, message: 'Failed to send email', error: err.message }).code(500);
            }
        }
    }
];
